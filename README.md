...
### 5. 거래소 수익 예측 모델
* 목적:  
  - 거래소의 주요 수익원인 거래량과 수수료를 기반으로 거래소 예상 수익을 예측
  - 단순한 거래량 × 수수료율 공식을 넘어서, 거래량 변화율 및 시장 흐름을 반영하여 더 정확한 예측 가능
  - 거래소의 핵심 Business KPI를 분석하여, 트레이딩 활성화와 수익성 간의 관계를 도출
 
* 데이터
  모델 학습 향상을 위해 기존 분석 일주일에서 30일치 데이터로 확장

* 분석한 Business KPI:

  - 거래소 수익성 분석: 거래량과 거래소 수익 간의 상관관계 분석
  - 거래량 변화율이 수익성에 미치는 영향: 급격한 거래량 변동이 거래소 수익에 미치는 영향 도출
  - 장기적 트렌드(7일 평균 거래량)와 거래소 수익 간의 관계: 단기적 거래량 급등이 아니라, 지속적인 거래량 상승이 수익성에 미치는 영향 분석
 ...
 * 회귀 분석 모델 설명  
    - 사용한 변수 (독립 변수 X)
      ...
      |변수|설명|추가한 이유|
      |------|---|---|
      |거래량|하루 동안의 총 거래량|거래소 수익 계산의 기본 요소|
      |거래량 변화율|거래량이 전일 대비 얼마나 변했는지|시장 변동성이 수익에 미치는 영향 반영|
      |7일 평균 거래량|최근 7일 동안 평균적으로 거래된 양|일시적 증가 vs. 장기적 상승을 구분|


    - 예측 모델 공식 (다중 회귀 분석)
      거래소 수익 = (거래량 × 계수₁) + (거래량 변화율 × 계수₂) + (7일 평균 거래량 × 계수₃) + 절편  
      현재 모델에서는 절편이 존재하지 않지만, 거래소가 기본적인 수익이 있을 때 절편으로 추가 (ex. 상장 수수료, API 제공 수익 등)
      
  - 모델의 한계 및 개선 가능성
    - 모델의 한계
      현재 모델은 거래량과 시장 변동성을 반영하지만, 실제 거래소의 수수료율 변동을 직접 반영하지 않음.
      거래소별로 고객 등급, VIP 할인, 마켓 메이커 정책, 특정 코인 이벤트 수수료 할인 등이 적용될 수 있음.
      하지만 무료 API에서는 거래소의 세부 수수료 데이터를 제공하지 않음, 따라서 모델에 반영하기 어려움.
  
    - 해결 방법 (추후 개선 가능성)
      거래소가 제공하는 데이터가 있다면,
      평균 거래 수수료율 변수를 추가하여 실제 수수료 변동 반영
      거래 유형별 비율 (마켓 메이커 vs 테이커 거래 비율)을 추가하여 수익 패턴 개선
      이벤트 여부 (특정 날 수수료 할인 등)를 반영하여 더 정교한 모델 구축 가능
      ✔ 하지만, 본 프로젝트는 무료 API를 기반으로 한 분석이므로, 이 한계를 명확히 인식하고 진행하였음.
